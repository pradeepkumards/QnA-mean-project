...
function createGroup(userId, newGroupObj, topicId, done) {
...
function deleteGroup(userId, groupId, done) {
  ...
  groupsModel.findOneAndRemove({
    ...
    console.log(deletedGroupObj)
    if (deletedGroupObj == null){
        console.log("Nothing to delete")
        return done("Your are not permitted to delete");
      }
...
// Handler to get a groups by topicId
function getGroupBytopicId(userId, topicId, done) {
  logger.info("Inside DAO method - get list questions/groups under a topic", topicId,userId);
  groupsModel.find({
    // userId: userId,
    topicId: topicId
  }, (err, groupObj) => {
