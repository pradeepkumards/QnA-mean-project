...
const notesDAO = require('./topics.dao');
...
describe('Notes DAO Layer Test Cases', function() {
  ...
  describe('Test scenarios for topics', function() {
    it('Add new valid note from mock for a user', function(done) {
      firstNoteId = uuidv4();
      mockNotesObj.noteId = firstNoteId;
      const userId = uuidv4();
      firstuserId = "73419b4c-a582-4c52-909c-f34519a1447b";
      // firstuserId = userId;
      // title = "Science"
      notesDAO.getGroups(firstuserId, (err, response) => {
        if (err) return done(err);
